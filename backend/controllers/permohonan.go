...
func GetJenisLayanan(c *gin.Context) {
	...
	config.DB.Where("is_active = 1").Find(&list)
...
func saveUploadedFile(c *gin.Context, fieldName, folder string) (string, error) {
	file, err := c.FormFile(fieldName)
	if err != nil {
		return "", nil
	}
	ext := filepath.Ext(file.Filename)
	filename := fmt.Sprintf("assets/%s/%d%s", folder, time.Now().UnixNano(), ext)
...
func CreatePermohonan(c *gin.Context) {
	...
	fileKTP, _ := saveUploadedFile(c, "file_ktp", "permohonan/ktp")
	fileKK, _ := saveUploadedFile(c, "file_kk", "permohonan/kk")
	filePendukung, _ := saveUploadedFile(c, "file_pendukung", "permohonan/pendukung")

	now := time.Now()
	p := models.Permohonan{
		NIK: userID.(string), TglPermohonan: now,
		Keterangan: keterangan, FileKTP: fileKTP,
		FileKK: fileKK, FilePendukung: filePendukung,
		Status: "verifikasi_persyaratan", CreatedAt: &now, UpdatedAt: &now,
	}
	fmt.Sscanf(idJenis, "%d", &p.IDJenisLayanan)
...
func UpdatePermohonan(c *gin.Context) {
	...
	if catatan != "" { p.CatatanPetugas = catatan }
	fileHasil, _ := saveUploadedFile(c, "file_hasil", "permohonan/hasil")
	if fileHasil != "" { p.FileHasil = fileHasil }
